...
interface CatpediaApiService {
    ...
    fun getListOfCatBreeds(): Flowable<List<BreedInfoResponse>>

    @GET(BuildConfig.ENDPOINT_SEARCH)
    fun getBreedImage(@Query(BuildConfig.QUERY_BREED_ID) breedId: String,
                      @Query(BuildConfig.QUERY_SIZE) size: String = "thumb"): Flowable<List<BreedImageResponse>>
    ...
    fun loginUser(@Query(BuildConfig.QUERY_USERNAME) username: String,
                  @Query(BuildConfig.QUERY_PASSWORD) password: String): Flowable<LoginResponse>
}